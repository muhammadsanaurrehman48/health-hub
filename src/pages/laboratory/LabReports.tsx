...
import { Search, Printer, Eye, FileText, Loader2 } from 'lucide-react';
...
const LabReports: React.FC = () => {
  ...
  const completedReports = labRequests.filter(r => r.status === 'completed');
  ...
  const handlePrint = (report: any) => {
    ...
    const html = [
      '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Lab Report - ' + report.requestNo + '</title>',
      '<style>',
      ...
      '<div><div class="label">Test</div><div class="value">' + (report.test || '-') + '</div></div>',
      '<div><div class="label">Requested By</div><div class="value">' + (report.doctor || '-') + '</div></div>',
      '</div>',
      '<div class="result-section"><h3>Test Results</h3>',
      '<div class="result-text">' + (typeof report.result === 'string' ? report.result : JSON.stringify(report.result || 'No results recorded', null, 2)) + '</div></div>',
      ...
      <div className="space-y-6">
        ...
        <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Lab Report — {viewingReport?.requestNo}</DialogTitle>
            </DialogHeader>
            {viewingReport && (
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-3 bg-muted/20 rounded-lg text-sm whitespace-pre-wrap">
                    {typeof viewingReport.result === 'string'
                      ? viewingReport.result
                      : JSON.stringify(viewingReport.result || 'No results recorded', null, 2)}
                  </div>
                </div>
              </div>
            )}
